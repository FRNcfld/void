...
public class HandshakeHandler
{
    ...
    void handleServerModListOnClient(HandshakeMessages.S2CModList serverModList, Supplier<NetworkEvent.Context> c)
    {
        ...
        if (!mismatchedChannels.isEmpty()) {
            ...
            c.get().getNetworkManager().m_129507_(Component.m_237113_("Connection closed - mismatched mod channel list"));
            return;
        }
        ...
        for (ResourceKey<? extends Registry<?>> key : serverModList.getCustomDataPackRegistries())
        {
            if (!clientDataPackRegistries.contains(key))
            {
                ...
                missingDataPackRegistries.add(key.m_135782_().toString());
            }
        }
        if (!missingDataPackRegistries.isEmpty())
        {
            c.get().getNetworkManager().m_129507_(Component.m_237110_("fml.menu.multiplayer.missingdatapackregistries", String.join(", ", missingDataPackRegistries)));
            return;
        }
    ...
    public boolean tickServer()
    {
        if (!negotiationStarted) {
            GameProfile profile = ((ServerLoginPacketListenerImpl) manager.m_129538_()).f_10021_;
